...
<section class="bg-white border rounded-xl mb-6">
    <header class="px-6 py-4 border-b">
        <h2 class="text-lg font-semibold text-gray-800">Employees with Assigned RFID Cards</h2>
        <p class="text-sm text-gray-500 mt-1">Active RFID assignments sourced from the employees table and matched to active cards.</p>
    </header>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                <?php if (empty($rfidAssignedCardRows)): ?>
                    <tr><td class="px-4 py-3 text-gray-500" colspan="6">No RFID card assignments found.</td></tr>
                <?php else: ?>
                            ...
                            <td class="px-4 py-3">
                                <?php if (($row['status_raw'] ?? '') === 'active'): ?>
                                    <form method="POST" action="timekeeping.php" class="inline-flex">
                                        ...
                                        <input type="hidden" name="assignment_source" value="<?= htmlspecialchars((string)($row['assignment_source'] ?? 'rfid_cards'), ENT_QUOTES, 'UTF-8') ?>">
                                        <input type="hidden" name="person_id" value="<?= htmlspecialchars((string)($row['person_id'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
...
<section class="bg-white border rounded-xl mb-6">
        ...
        <table id="rfidRecentEventsTable" class="w-full text-sm">
            ...
            <tbody class="divide-y">
                ...
                <?php else: ?>
                    ...
                    <?php endforeach; ?>
                    <tr id="rfidRecentEventsEmptyRow" class="hidden">
                        <td class="px-4 py-3 text-gray-500" colspan="7">No RFID scan events found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    ...
    <div class="px-6 pb-4 flex items-center justify-between gap-3">
        <p id="rfidRecentEventsPaginationInfo" class="text-xs text-slate-500">Page 1 of 1</p>
        <div class="flex items-center gap-2">
