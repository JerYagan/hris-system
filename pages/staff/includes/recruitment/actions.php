    ...
    function staffRecruitmentLoadEmailTemplates(string $supabaseUrl, array $headers): array
    {
        $defaults = [
            ...
            'next_stage' => [
                ...
                'body' => 'Hello {applicant_name},<br><br>Your application for <strong>{job_title}</strong> has moved to the next stage.<br>Reference: <strong>{application_ref_no}</strong><br><br>Remarks: {remarks}<br><br>Please monitor your account and email for final review schedule and division signing instructions.',
            ],
        ];
...
if ($action === 'create_job_posting') {
    ...
    if ($title === '' || $description === '' || $officeId === '' || $positionId === '' || $openDate === '' || $closeDate === '') {
        redirectWithState('error', 'Title, division, position, description, open date, and close date are required.');
    }

    if (!isValidUuid($officeId) || !isValidUuid($positionId)) {
        redirectWithState('error', 'Selected division or position is invalid.');
    }
...
if ($action === 'add_hired_applicant_as_employee') {
    ...
    if (!$isAdminScope && isValidUuid((string)$staffOfficeId) && strtolower((string)$staffOfficeId) !== strtolower($officeId)) {
        redirectWithState('error', 'You are not allowed to add employees outside your division scope.');
    }
    ...
    $personResponse = apiRequest(
        'GET',
        $supabaseUrl
        . '/rest/v1/people?select=id,user_id,surname,first_name,personal_email,mobile_no,agency_employee_no'
    ...
    if (!isValidUuid($personId)) {
        ...
        $employeeId = staffRecruitmentGenerateEmployeeId($supabaseUrl, $headers);
        ...
        if ($employeeId === '') {
            $employeeId = staffRecruitmentGenerateEmployeeId($supabaseUrl, $headers);
            $peoplePatch['agency_employee_no'] = $employeeId;
        }
...
if ($action === 'unarchive_job_posting') {
    ...
    if (!is_array($postingRow)) {
        redirectWithState('error', 'Archived posting not found or outside your division scope.');
    }
...
if ($action === 'save_applicant_decision') {
    ...
    if (!$isAdminScope && isValidUuid((string)$staffOfficeId)) {
        ...
        if (!is_array($scopeRow)) {
            redirectWithState('error', 'You are not allowed to process applications outside your division scope.');
        }
    }
...
$postingRow = isSuccessful($postingResponse) ? ($postingResponse['data'][0] ?? null) : null;
if (!is_array($postingRow)) {
    redirectWithState('error', 'Posting not found or outside your division scope.');
}
