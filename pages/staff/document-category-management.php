...
$pageTitle = 'Document Category Management | Staff';
...
<div class="mb-6 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Document Category Management</h1>
        <p class="mt-1 text-sm text-gray-500">Add, rename, and remove document upload categories. Categories that are still attached to documents cannot be deleted.</p>
    </div>
    <a href="document-management.php" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
...
<section class="mb-6 grid grid-cols-1 gap-3 md:grid-cols-3">
    ...
    <article class="rounded-xl border border-slate-300 bg-slate-100 px-4 py-3">
...
<section class="mb-6 rounded-xl border bg-white">
    <header class="border-b px-6 py-4">
        <h2 class="text-lg font-semibold text-gray-800">Add Category</h2>
        <p class="mt-1 text-sm text-gray-500">New categories become available in document upload and review workflows.</p>
    </header>
    <form action="document-category-management.php" method="POST" class="px-6 py-5 md:flex md:items-start md:gap-4">
        ...
        <div class="min-w-0 flex-1">
            ...
            <input type="text" name="category_name" maxlength="80" class="mt-1 w-full rounded-md border px-3 py-2 text-sm" placeholder="Examples: Certification, Compliance Letter" required>
            <p class="mt-1 text-xs text-gray-500">Letters, numbers, spaces, and basic punctuation are allowed. Placeholder values such as haugafia are blocked.</p>
        </div>
        <div class="mt-4 md:mt-7 md:flex-none">
            <button type="submit" class="w-full rounded-md bg-green-700 px-6 py-2 text-sm text-white hover:bg-green-800 md:w-auto md:min-w-[220px]">Add Category</button>
        </div>
    </form>
</section>
...
<section class="rounded-xl border bg-white">
    <header class="border-b px-6 py-4">
        ...
        <p class="mt-1 text-sm text-gray-500">Rename categories inline. Delete is available only when no document still references the category.</p>
    </header>
    <div class="overflow-x-auto px-6 py-5">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                <?php if ($categoryRows === []): ?>
                    <tr>
                        <td class="px-4 py-4 text-gray-500" colspan="5">No document categories found.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($categoryRows as $categoryRow): ?>
                        ...
                        <tr>
                            <td class="px-4 py-4 align-top">
                                <form action="document-category-management.php" method="POST" class="space-y-3 md:flex md:items-start md:gap-3 md:space-y-0">
                                    ...
                                    <div class="min-w-[240px] flex-1">
                                        <input type="text" name="category_name" value="<?= htmlspecialchars((string)($categoryRow['category_name'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" maxlength="80" class="w-full rounded-md border px-3 py-2 text-sm" required>
                                    </div>
                                    <button type="submit" class="rounded-md border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50">Save</button>
                                </form>
                            </td>
                            <td class="px-4 py-4 align-top font-mono text-xs text-gray-600"><?= htmlspecialchars((string)($categoryRow['category_key'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-4 py-4 align-top text-gray-700">
                                ...
                                <span class="block text-xs text-gray-500">Active: <?= htmlspecialchars((string)$activeUsage, ENT_QUOTES, 'UTF-8') ?></span>
                            </td>
                            <td class="px-4 py-4 align-top text-gray-600"><?= htmlspecialchars($createdAt !== '' ? date('M j, Y', strtotime($createdAt)) : '-', ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-4 py-4 align-top">
                                <?php if ($totalUsage > 0): ?>
                                    <p class="text-xs text-gray-500">Remove unavailable while documents still use this category.</p>
                                <?php else: ?>
