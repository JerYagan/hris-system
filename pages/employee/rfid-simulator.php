...
$feedbackMessage = $message ?? 'Submit a simulated tap to see whether your RFID card was processed as time-in, time-out, or ignored as a duplicate.';
...
<div class="mb-6 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Simulate an RFID tap for your assigned card using the same attendance write path used by the scanner endpoint.</p>
    </div>
...
<section class="mb-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
    ...
    <article class="rounded-xl border bg-white px-5 py-4 shadow-sm">
        <p class="text-xs uppercase tracking-wide text-slate-500">Attendance Visibility</p>
        <p class="mt-2 text-sm text-slate-700">Simulator taps write to <span class="font-medium">attendance_logs</span>, so the resulting record appears in employee, staff, and admin timekeeping views.</p>
    </article>
</section>
...
<section class="rounded-xl border bg-white shadow-sm">
    <header class="border-b px-6 py-4">
        ...
        <p class="mt-1 text-sm text-slate-500">The backend will decide whether this tap logs <span class="font-medium">time-in</span> or <span class="font-medium">time-out</span> for today.</p>
    </header>
...
(() => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        applyFeedbackState(
            ...
            'Your simulated RFID tap is being processed now. This uses the same attendance flow used by the RFID endpoint.',
