...
<?php if (!empty($message)): ?>
    <section
        class="mb-6 rounded-xl border px-4 py-3 text-sm <?= ($state ?? '') === 'success' ? 'border-green-200 bg-green-50 text-green-800' : 'border-rose-200 bg-rose-50 text-rose-800' ?>">
...
<?php if (!empty($dataLoadError)): ?>
    <section class="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="<?= $retryUrl ?>"
                class="inline-flex items-center justify-center rounded-md border border-amber-300 bg-white px-3 py-1.5 text-xs text-amber-800 hover:bg-amber-100">Retry</a>
        </div>
    </section>
<?php endif; ?>
        ...
        <div class="w-full max-w-lg rounded-xl border bg-white p-5 shadow-xl">
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600">Your profile is missing required background details needed for
                        automatic qualification checks.</p>
                </div>
                <button type="button" id="closeProfileCompletionModal"
                    class="rounded-md border px-2 py-1 text-xs text-gray-600 hover:bg-gray-50">Close</button>
            </div>

            <div class="mt-4 space-y-2 text-sm">
                <p
                    class="rounded-md border px-3 py-2 <?= ((int) ($profileCompletionReminder['education_entries'] ?? 0) > 0) ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-rose-200 bg-rose-50 text-rose-800' ?>">
                    Education Entries: <?= (int) ($profileCompletionReminder['education_entries'] ?? 0) ?>
                </p>
                <p
                    class="rounded-md border px-3 py-2 <?= ((int) ($profileCompletionReminder['work_entries'] ?? 0) > 0) ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-rose-200 bg-rose-50 text-rose-800' ?>">
                    Work Experience Entries: <?= (int) ($profileCompletionReminder['work_entries'] ?? 0) ?>
                </p>
            </div>

            <div class="mt-5 flex flex-wrap justify-end gap-2">
                <button type="button" id="laterProfileCompletionModal"
                    class="inline-flex items-center rounded-md border px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">Remind
                    Me Later</button>
                <a href="profile.php?edit=true"
                    class="inline-flex items-center rounded-md bg-green-700 px-3 py-2 text-sm font-medium text-white hover:bg-green-800">Go
                    to Profile</a>
            </div>
        </div>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border bg-gradient-to-r from-green-50 via-white to-green-50 p-4 sm:p-5">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <p class="text-sm text-green-700 font-medium">Welcome back,
                        <?= htmlspecialchars((string) $dashboardData['first_name'], ENT_QUOTES, 'UTF-8') ?></p>
                    <h1 class="mt-1 text-xl font-semibold text-gray-800 sm:text-2xl">Your Recruitment Journey</h1>
                    <p class="mt-2 max-w-2xl text-sm text-gray-600">
                        Track your application, discover opportunities, and stay updated with HR announcements in one
                        place.
                    </p>
                    <div
                        class="mt-4 inline-flex items-center gap-1 rounded-full bg-yellow-100 px-3 py-1 text-xs font-medium text-yellow-800">
...
<section class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p class="mt-2 text-base font-semibold text-slate-800">
            <?= htmlspecialchars((string) $dashboardData['position_applied'], ENT_QUOTES, 'UTF-8') ?></p>
    </article>
    <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex items-center gap-2 text-xs uppercase tracking-wide text-slate-500">
        ...
        <p class="mt-2 text-base font-semibold text-slate-800">
            <?= htmlspecialchars((string) $dashboardData['current_stage'], ENT_QUOTES, 'UTF-8') ?></p>
    </article>
    <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex items-center gap-2 text-xs uppercase tracking-wide text-slate-500">
        ...
        <p class="mt-2 text-base font-semibold text-slate-800">
            <?= htmlspecialchars((string) $dashboardData['date_applied'], ENT_QUOTES, 'UTF-8') ?></p>
    </article>
    <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex items-center gap-2 text-xs uppercase tracking-wide text-slate-500">
        ...
        <p class="mt-2 text-base font-semibold text-slate-800">
            <?= htmlspecialchars((string) $dashboardData['latest_update'], ENT_QUOTES, 'UTF-8') ?></p>
    </article>
</section>
        ...
        <div class="p-5 sm:p-6">
            <?php if (empty($dashboardTimelineItems)): ?>
                <div class="rounded-lg border border-slate-200 bg-slate-50 p-5">
                    ...
                    <p class="mt-1 text-sm text-slate-600">Start by browsing open positions and submitting your first
                        application.</p>
                    <a href="job-list.php"
                        class="mt-3 inline-flex items-center gap-1 text-sm font-medium text-green-700 hover:underline">
                        Browse Open Jobs
            ...
            <?php else: ?>
                <div class="w-full max-w-full overflow-x-auto">
                    <ol class="grid min-w-[920px] gap-0" style="grid-template-columns: repeat(<?= (int) count($dashboardTimelineItems) ?>, minmax(0, 1fr));">
                        <?php foreach ($dashboardTimelineItems as $index => $timelineItem): ?>
                            ...
                            $isFirstItem = $index === 0;
                            $prevState = (string) (($dashboardTimelineItems[$index - 1]['state'] ?? 'upcoming'));
                            $nextState = (string) (($dashboardTimelineItems[$index + 1]['state'] ?? 'upcoming'));
                            $dotClass = 'border-slate-300 bg-white text-slate-400';
                            $leftLineClass = 'bg-slate-200';
                            $rightLineClass = 'bg-slate-200';
                            ...
                            if (!$isFirstItem && $prevState === 'completed' && in_array($timelineState, ['completed', 'current'], true)) {
                                $leftLineClass = 'bg-green-600';
                            }

                            if (!$isLastItem && $timelineState === 'completed' && in_array($nextState, ['completed', 'current'], true)) {
                                $rightLineClass = 'bg-green-600';
                            }
                            ?>
                            <li class="min-w-0 text-center">
                                <p class="mb-3 min-h-[20px] text-sm font-semibold text-slate-700">
                                    <?= htmlspecialchars((string) ($timelineItem['date_label'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                </p>
                                <div class="relative flex items-center justify-center">
                                    <?php if (!$isFirstItem): ?>
                                        <span class="absolute left-0 right-1/2 h-2 <?= $leftLineClass ?>"></span>
                                    <?php endif; ?>
                                    <span
                                        class="relative z-10 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 text-sm <?= $dotClass ?>">
                                        <span class="material-symbols-outlined text-base">
                                            <?= $iconName ?>
                                        </span>
                                    </span>
                                    <?php if (!$isLastItem): ?>
                                        <span class="absolute left-1/2 right-0 h-2 <?= $rightLineClass ?>"></span>
                                    <?php endif; ?>
                                </div>
                                <p class="mt-3 text-sm font-semibold leading-5 <?= $titleClass ?>">
                                    <?= htmlspecialchars((string) ($timelineItem['title'] ?? 'Application Step'), ENT_QUOTES, 'UTF-8') ?>
                                </p>
                                <p class="mt-1 text-xs leading-5 text-slate-500">
                                    <?= htmlspecialchars((string) ($timelineItem['subtitle'] ?? '-'), ENT_QUOTES, 'UTF-8') ?>
                                </p>
                            </li>
                        <?php endforeach; ?>
                    </ol>
                            ...
                            <div class="flex-1">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <h4 class="font-medium text-gray-800">
                                        <?= htmlspecialchars((string) $item['title'], ENT_QUOTES, 'UTF-8') ?></h4>
                                    <span
                                        class="text-xs text-gray-500"><?= !empty($item['created_at']) ? htmlspecialchars(date('M j, Y', strtotime((string) $item['created_at'])), ENT_QUOTES, 'UTF-8') : '-' ?></span>
                                </div>
                                <p class="mt-1 text-sm text-gray-600">
                                    <?= htmlspecialchars((string) $item['body'], ENT_QUOTES, 'UTF-8') ?></p>
                                <?php if (!empty($item['link_url'])): ?>
                                    <a href="<?= htmlspecialchars((string) $item['link_url'], ENT_QUOTES, 'UTF-8') ?>"
                                        class="mt-2 inline-flex items-center gap-1 text-sm text-green-700 hover:underline">
                                        Open update
                                        <span class="material-symbols-outlined text-sm">arrow_forward</span>
                                    </a>
                                <?php endif; ?>
...
<section class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
    <div class="rounded-xl border bg-white">
        ...
        <div class="grid grid-cols-1 gap-4 p-6 sm:grid-cols-2">
            <a href="job-list.php"
                class="group rounded-xl border bg-white p-4 transition hover:border-green-600 hover:bg-green-50/40">
    ...
    <div class="rounded-xl border bg-white">
        ...
        <div class="space-y-3 p-6 text-sm text-gray-600">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="profile.php?edit=true"
                    class="inline-flex items-center gap-1 rounded-md border px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">
