...
if ($action === 'create_award_category') {
    ...
    apiRequest(
        ...
        [[
            ...
            'old_data' => null,
            'new_data' => ['award_code' => strtoupper($awardCode), 'award_name' => $awardName, 'criteria' => $criteria, 'is_active' => $isActive],
            'ip_address' => clientIp(),
        ]]
    );
...
if ($action !== '') {
    redirectWithState('error', 'Unknown awards and recognition action.');
}
