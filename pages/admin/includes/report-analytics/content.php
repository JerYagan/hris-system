...
$reportAnalyticsRenderSummary = in_array($reportAnalyticsDataStage, ['summary', 'full'], true);
$reportAnalyticsRenderSecondary = in_array($reportAnalyticsDataStage, ['secondary', 'full', 'workforce', 'demographics', 'turnover', 'operations'], true);
$reportAnalyticsRenderWorkforce = in_array($reportAnalyticsDataStage, ['secondary', 'full', 'workforce'], true);
$reportAnalyticsRenderDemographics = in_array($reportAnalyticsDataStage, ['secondary', 'full', 'demographics'], true);
$reportAnalyticsRenderTurnover = in_array($reportAnalyticsDataStage, ['secondary', 'full', 'turnover'], true);
$reportAnalyticsRenderOperations = in_array($reportAnalyticsDataStage, ['secondary', 'full', 'operations'], true);
...
$attendanceVariance = round((float)($attendanceComplianceCurrent ?? 0) - (float)($attendanceCompliancePrevious ?? 0), 1);
$lateVariance = (int)($attendanceCurrent['late'] ?? 0) - (int)($attendancePrevious['late'] ?? 0);
$grossVariance = (float)($payrollCurrent['gross'] ?? 0) - (float)($payrollPrevious['gross'] ?? 0);
$netVariance = (float)($payrollCurrent['net'] ?? 0) - (float)($payrollPrevious['net'] ?? 0);
?>

<?php if ($reportAnalyticsRenderSummary): ?>
    ...
    <section class="bg-white border border-slate-200 rounded-2xl mb-6">
        <header class="px-6 py-4 border-b border-slate-200">
            ...
            <p class="text-sm text-slate-500 mt-1">Headline workforce KPIs render first while charts and report tables continue loading.</p>
        </header>
        ...
        <div class="px-6 pb-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <article class="report-analytics-chart-card rounded-xl border border-slate-200 p-4">
    ...
    <section class="bg-white border border-slate-200 rounded-2xl mb-6">
        <header class="px-6 py-4 border-b border-slate-200">
            ...
            <p class="text-sm text-slate-500 mt-1">Aggregate-first counts across attendance, payroll, recruitment, exported reports, performance, and audit logs.</p>
        </header>
...
<?php if ($reportAnalyticsRenderSecondary): ?>
    <?php if (!empty($dataLoadError)): ?>
        <div class="mb-6 rounded-lg border border-red-200 bg-red-50 text-red-700 px-4 py-3 text-sm flex gap-2">
            <span class="material-symbols-outlined text-base">error</span>
            <span><?= htmlspecialchars((string)$dataLoadError, ENT_QUOTES, 'UTF-8') ?></span>
    ...
    <?php if ($reportAnalyticsRenderWorkforce): ?>
    <section class="bg-white border border-slate-200 rounded-2xl mb-6">
        <header class="px-6 py-4 border-b border-slate-200">
            <h2 class="text-lg font-semibold text-slate-800">Workforce Breakdown</h2>
            <p class="text-sm text-slate-500 mt-1">Charts and searchable employee records load after the KPI shell so first paint stays light.</p>
        </header>

        <div class="px-6 py-6 grid grid-cols-1 xl:grid-cols-2 gap-4">
            <article class="rounded-xl border border-slate-200 p-4">
                ...
                <div class="report-analytics-chart-frame relative h-72">
    ...
    <?php if ($reportAnalyticsRenderDemographics): ?>
    <section class="bg-white border border-slate-200 rounded-2xl mb-6">
        <header class="px-6 py-4 border-b border-slate-200">
        ...
        <div class="px-6 pt-6">
            <article class="report-analytics-chart-card rounded-xl border border-slate-200 p-4">
                ...
                <div class="report-analytics-chart-frame relative h-80">
    ...
    <?php if ($reportAnalyticsRenderTurnover): ?>
    <section class="bg-white border border-slate-200 rounded-2xl mb-6">
        <header class="px-6 py-4 border-b border-slate-200">
    ...
    <?php if ($reportAnalyticsRenderOperations): ?>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr>
                        ...
                        <td class="px-4 py-3"><?= htmlspecialchars(number_format((float)($attendanceComplianceCurrent ?? 0), 1), ENT_QUOTES, 'UTF-8') ?>%</td>
                        <td class="px-4 py-3"><?= htmlspecialchars(number_format((float)($attendanceCompliancePrevious ?? 0), 1), ENT_QUOTES, 'UTF-8') ?>%</td>
                    ...
                    <?php if (!(bool)($noLatePolicyApproved ?? false)): ?>
                        <tr>
                            ...
                            <td class="px-4 py-3"><?= htmlspecialchars((string)($attendanceCurrent['late'] ?? 0), ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-4 py-3"><?= htmlspecialchars((string)($attendancePrevious['late'] ?? 0), ENT_QUOTES, 'UTF-8') ?></td>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3"><?= htmlspecialchars($money((float)($payrollCurrent['gross'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></td>
                        <td class="px-4 py-3"><?= htmlspecialchars($money((float)($payrollPrevious['gross'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></td>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3"><?= htmlspecialchars($money((float)($payrollCurrent['net'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></td>
                        <td class="px-4 py-3"><?= htmlspecialchars($money((float)($payrollPrevious['net'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></td>
    ...
    <section class="bg-white border border-slate-200 rounded-2xl mb-6">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 text-sm">
            <article class="rounded-xl border border-slate-200 p-4 bg-sky-50">
                ...
                <p class="text-2xl font-bold text-slate-800 mt-2"><?= htmlspecialchars((string)((int)($advancedAdminAnalytics['demographics_male'] ?? 0) + (int)($advancedAdminAnalytics['demographics_female'] ?? 0) + (int)($advancedAdminAnalytics['demographics_unspecified'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></p>
            ...
            <article class="rounded-xl border border-slate-200 p-4 bg-violet-50">
                ...
                <p class="text-2xl font-bold text-slate-800 mt-2"><?= htmlspecialchars((string)((int)($advancedAdminAnalytics['admin_activity_30_days'] ?? 0) + (int)($advancedAdminAnalytics['staff_activity_30_days'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></p>
    ...
    <section class="bg-white border border-slate-200 rounded-2xl">
        <header class="px-6 py-4 border-b border-slate-200">
            ...
            <p class="text-sm text-slate-500 mt-1">Export tools stay outside the first paint path and only initialize after analytics sections load.</p>
        </header>
